...
A new function that will place to %ProgramData%\HardenAD\Configuration any reference files for later uses (such as scheduling).
...
A new GPO that deploy a schedule on the PDC to hunt for rogue AdminCount objects and sanitize them every sunday at 8AM.
The GPO uses the script "Clear-AdminCount.ps1", refers to it for details.
