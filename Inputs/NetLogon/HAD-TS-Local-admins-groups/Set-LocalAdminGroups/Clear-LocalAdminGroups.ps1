...
#revoir pour prendre les 2 types de groupes 
if ($mode -eq "LOCALADMIN" -or $mode -eq "BOTH")
{
    $GroupNameFilter = "$($myConfig.customRuleSet.config.PrefixDomLoc)$(($myConfig.customRuleSet.config.settings.LOCALADMIN.GroupName).REPLACE('%ComputerName%',''))"
...
{
    $GroupNameFilter = "$($myConfig.customRuleSet.config.PrefixDomLoc)$(($myConfig.customRuleSet.config.settings.RDU.GroupName).REPLACE('%ComputerName%',''))"
    # Computing group name filter
    $debugMessage += Write-DebugLog INF "GROUP FILTER: $GroupNameFilter"
