...
if($status -eq "Initialized" -and $pincode -eq "0000" -and $s_status -eq "Running"){
    ...
    while($status -eq "Initialized"){
        ...
        $label.Text = "Pour des raisons de s�curit�, votre poste de travail va �tre chiffr�.`nDe ce fait, vous allez devoir choisir un code PIN d'au moins 6 caract�res.`nCe code PIN vous sera demand� (apr�s plusieurs red�marrages) � l'allumage de votre poste.`nVotre code PIN ne peut contenir que les chiffres de 0 � 9.`nVotre poste red�marrera automatiquement apr�s le renseignement du code PIN, veuillez sauvegarder vos donn�es avant de confirmer votre code PIN."
        ...
        if($result -eq [System.Windows.Forms.DialogResult]::OK)
        {
            ...
            if($codepin1 -eq $codepin2 -and $codepin1 -ne "" -and $codepin1 -ne "9999" -and $codepin1 -ne "0000" -and $codepin1.Length -ge 6 -and $codepin1 -match "^\d+$" -and $(Get-Connectivity) -and $s_status -eq "Running"){
                ...
                if($valuePIN -eq $codepin1){
                    ...
                    $msgBoxInput = [System.Windows.MessageBox]::Show("Votre code PIN est inf�rieur � 6 caract�res ou votre code PIN ne contient pas uniquement des chiffres entre 0 et 9.`nVeuillez renseigner un code PIN valide.","Attention !",'OK','Error')
                }
            }elseif($s_status -ne "Running"){
                "[$(Get-Date)][$($active_script_name)] Le service HADBLO02 n'est pas d�marr�" | Out-File -Append -FilePath $log_file
                Add-Type -AssemblyName PresentationFramework
                $msgBoxInput = [System.Windows.MessageBox]::Show("Une erreur est survenue.`nLe service HADBLO02 n'est pas d�marr�.`nVeuillez red�marrer votre poste pour r�essayer.","Attention !",'OK','Error')
                "[$(Get-Date)][$($active_script_name)] END" | Out-File -Append -FilePath $log_file
                exit
            }elseif(!$(Get-Connectivity)){
                "[$(Get-Date)][$($active_script_name)] Le poste n'est pas connect� au domaine. Veuillez le reconnecter et reessayer." | Out-File -Append -FilePath $log_file
                Add-Type -AssemblyName PresentationFramework
                $msgBoxInput = [System.Windows.MessageBox]::Show("Le poste de travail n'est pas connect� au domaine`nVeuillez v�rifier votre connexion VPN si vous �tes � distance.","Attention !",'OK','Error')
            }else{
                ...
                Add-Type -AssemblyName PresentationFramework
                $msgBoxInput = [System.Windows.MessageBox]::Show("Les code PIN renseign�s ne sont pas identiques ou sont nuls`nVeuillez renseigner un code PIN valide.","Attention !",'OK','Error')
            }
