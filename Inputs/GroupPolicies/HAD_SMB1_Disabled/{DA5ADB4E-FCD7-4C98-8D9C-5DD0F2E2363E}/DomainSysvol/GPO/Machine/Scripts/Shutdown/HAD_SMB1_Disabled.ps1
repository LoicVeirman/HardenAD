...
try{
    ...
    if($LastExitCode -eq 0){
        ...
        "[$(Get-Date)][$($active_script_name)] Disable SMB1 client failed with exitcode $($LastExitCode): $($Error[0])" | Out-File -Append -FilePath $hardenAD_logs_path
    }    
}catch{
    "[$(Get-Date)][$($active_script_name)] Disable SMB1 client failed: $($Error[0])" | Out-File -Append -FilePath $hardenAD_logs_path
}


#if os > windows 10 or windows 2012
if($os -ge [version]"6.2"){
    ...
    if($smb1.State -eq "Enabled"){
        $Error.Clear()
        try{
            Disable-WindowsOptionalFeature -Online -FeatureName SMB1Protocol -NoRestart -all
